              ...
              <div class="bg-gray-100 p-6 rounded-lg">
                ...
                <ul class="space-y-3">
                  ...
                  <li class="flex items-center">
                    ...
                    <a :href="company.link" class="nav-link text-gray-700 hover:text-blue-600" target="_blank"
                      rel="noopenes noreferrer">Ir al proyecto</a>
                  </li>
                </ul>
              </div>
            </div>
  ...
  <div v-else>
    <div class="flex items-center justify-center min-h-screen">
      <div class="text-center mt-10">
        ...
        <button @click.prevent="goToSection('hero')" class=" items-center text-blue-600 hover:text-blue-800 transition-colors">
          <i class="fas fa-arrow-left mr-2"></i> Regresar
        </button>
      </div>
    </div>
...
export default {
  ...
  async created() {
    ...
    try {
      ...
      if (res.status !== 200 || !data || (Array.isArray(data) && data.length === 0)) {
        this.company = null;
      } else {
